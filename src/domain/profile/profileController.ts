...
export const updateData = async (data: updateData) => {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      // Из за асинхронного поведения оставлю такой подход с перезагрузкой страницы
      // Исходя из того, что в чеклисте сказано:
      // При перезагрузке страницы по F5 (⌘+R) отправленное только что сообщение должно(!) отображаться в чате.(если я все правильно понял)
      // Я могу предположить, что обновление данных пользователя так же можно сделать по перезагрузке страницы, что я форсирую для отображения данных
    }
    console.log(response.responseText);
  } catch (error) {
    console.log(error);
